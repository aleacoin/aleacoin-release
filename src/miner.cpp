...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    int64_t nFees = 0;
    {
        ...
        if (!fProofOfStake)
        {
            pblock->vtx[0].vout[0].nValue = round(GetProofOfWorkReward(nFees) * (1 - devCoin) / 10000) * 10000;
            pblock->vtx[0].vout[1].nValue = GetProofOfWorkReward(nFees) - pblock->vtx[0].vout[0].nValue;
        }
