...
float devCoin = 0.015; //1% to foundation
...
const string strMessageMagic = "Aleacoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // aleacoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // aleacoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetRandomReward(int maxReward)
{
	CBlock block;
	const CBlockIndex* pindex;
	pindex = FindBlockByHeight(pindexBest->nHeight);
	block.ReadFromDisk(pindex);
	std::string cseed_str = block.GetHash().ToString().substr(7,7);
	const char* cseed = cseed_str.c_str();
	long seed = hex2long(cseed);
	int randomReward = generateMTRandom(seed, maxReward-1) + 1;
		
	return randomReward * COIN;	
}
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	{
		int PoWStopRewardLimit = 0;
		
		if(pindexBest->nHeight < 14000)
		{
			PoWStopRewardLimit = 5;
		}
		else if (pindexBest->nHeight < 18000)
		{
			PoWStopRewardLimit = 10;
		}
		else if (pindexBest->nHeight < 22000)
		{
			PoWStopRewardLimit = 20;
		}
		else if (pindexBest->nHeight < 26000)
		{
			PoWStopRewardLimit = 30;
		}		
		else if (pindexBest->nHeight < 30000)
		{
			PoWStopRewardLimit = 40;
		}
		else if (pindexBest->nHeight < 34000)
		{
			PoWStopRewardLimit = 50;
		}
		
		int prevCounter = 0;
		CBlock block;
		const CBlockIndex* pindex;
		pindex = FindBlockByHeight(pindexBest->nHeight-prevCounter);
		
		while(pindex->IsProofOfStake())
		{
			prevCounter++;
			pindex = FindBlockByHeight(pindexBest->nHeight-prevCounter);
		}
		...
		block.ReadFromDisk(pindex);
		int64_t prevReward = block.vtx[0].GetValueOut();
		...
		block.ReadFromDisk(pindex);
		int64_t prevReward2 = block.vtx[0].GetValueOut();
		
		if((prevReward <= (PoWStopRewardLimit * COIN)) && (prevReward2 <= (PoWStopRewardLimit * COIN)))
		{
			nSubsidy = 1 * COIN;			
		}
		else
...
// aleacoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes Aleacoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // aleacoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // aleacoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if(IsProofOfWork())
    {
        ...
        if (vtx[0].vout[1].nValue < GetProofOfWorkReward(nFees) - vtx[0].vout[0].nValue)
            return error("ConnectBlock() : coinbase does not pay enough to dev addresss");
    }

    if (IsProofOfStake())
    {
        // aleacoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // aleacoin: track money supply and mint amount info
...
// aleacoin: total coin age spent in transaction, in the unit of coin-days.
...
// aleacoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // aleacoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // aleacoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // aleacoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // aleacoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // aleacoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // aleacoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // aleacoin: getblocks may not obtain the ancestor block rejected
    ...
    // aleacoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Aleacoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "1406016999";
        CTransaction txNew;
        txNew.nTime = 1406016999;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("3e27a53cdd90361ccb663169b2f781bb3c20becc5efc0c67b54a34f8e888d6ac"));
        block.print();
        ...
        // aleacoin: initialize synchronized checkpoint
...
void PrintBlockTree()
{
    ...
    while (!vStack.empty())
    {
		...
		/*
        printf("%d (%u,%u) %I64d %s  %08x  %s  mint %7s  tx %"PRIszu" \n",
            pindex->nHeight,
            pindex->nFile,
            pindex->nBlockPos,
		...
		*/
		...
		{		
			printf("Height=;%u;Reward=;%f;Stake\n",pindex->nHeight,block.vtx[0].GetValueOut());
		}
		else
		{
			printf("Height=;%u;Reward=;%I64d;Work\n",pindex->nHeight,block.vtx[0].GetValueOut());
		}
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // aleacoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // aleacoin: tell downloading node about the latest block if it's
